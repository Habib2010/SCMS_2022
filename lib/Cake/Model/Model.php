...
class Model extends Object implements CakeEventListener {
        ...
        foreach ($this->belongsTo as $parent => $assoc) {
            if (!empty($assoc['counterCache'])) {
                ...
                foreach ($assoc['counterCache'] as $field => $conditions) {
                    ...
                    if (isset($keys['old'][$foreignKey])) {
                        if ($keys['old'][$foreignKey] != $keys[$foreignKey]) {
                            ...
                            $this->{$parent}->updateAll(
                                    array($field => $count), array($this->{$parent}->escapeField() => $keys['old'][$foreignKey])
                            );
                        }
                    }
                    ...
                    $this->{$parent}->updateAll(
                            array($field => $count), array($this->{$parent}->escapeField() => $keys[$foreignKey])
                    );
                }
            }
        }
    ...
    public function getAssociated($type = null) {
        if ($type == null) {
            ...
            $assoc = array_merge(
                    $this->hasOne, $this->hasMany, $this->belongsTo, $this->hasAndBelongsToMany
            );
    ...
    public function joinModel($assoc, $keys = array()) {
        ...
        trigger_error(
                __d('cake_dev', 'Invalid join model settings in %s', $model->alias), E_USER_WARNING
        );
    }
