...
class TreeBehavior extends ModelBehavior {
    ...
    public function generatePurposeList(Model $Model, $conditions = null, $keyPath = null, $valuePath = null, $spacer = '_', $recursive = null) {
        $overrideRecursive = $recursive;
        extract($this->settings[$Model->alias]);
        if (!is_null($overrideRecursive)) {
            $recursive = $overrideRecursive;
    ...
    public function removeFromTree(Model $Model, $id = null, $delete = false) {
        ...
        $db = ConnectionManager::getDataSource($Model->useDbConfig);
        $Model->updateAll(
                array($parent => $db->value($node[$parent], $parent)), array($Model->escapeField($parent) => $node[$Model->primaryKey])
        );
        ...
        if ($delete) {
            ...
            $Model->id = $id;
            return $Model->save(
                            array($left => $edge + 1, $right => $edge + 2, $parent => null), array('callbacks' => false, 'validate' => false)
            );
        }
    }
    ...
    protected function _setParent(Model $Model, $parentId = null, $created = false) {
        ...
        if (empty($parentId)) {
            ...
            if (empty($node[$left]) && empty($node[$right])) {
                ...
                $result = $Model->save(
                        array($left => $parentNode[$right], $right => $parentNode[$right] + 1, $parent => $parentId), array('validate' => false, 'callbacks' => false)
                );
                $Model->data = $result;
            } else {
                $this->_sync($Model, $edge - $node[$left] + 1, '+', 'BETWEEN ' . $node[$left] . ' AND ' . $node[$right], $created);
