...
class UsersController extends AppController {
     ...
     * @access public
     */
    //public $uses = array('User','Employee','Student','Guardian');
    ...
    public function admin_add() {
        if (!empty($this->request->data)) {
            ...
            /* //Only for capabilities:
              $allCaps = array_filter($this->request->data['User']['capabilities']);
              if( !(empty($allCaps) || empty($allCaps['level']) || empty($allCaps['section'])) ){
              $this->request->data['User']['capabilities'] = implode('=>',$allCaps);//serialize($allCaps);
              }else{
              unset($this->request->data['User']['capabilities']);
              } */
    ...
    public function admin_edit($id = null) {
        if (!empty($this->request->data)) {
            ...
            /* //Only for capabilities:
              $allCaps = array_filter($this->request->data['User']['capabilities']);
              if( !(empty($allCaps) || empty($allCaps['level']) || empty($allCaps['section'])) ){
              $this->request->data['User']['capabilities'] = serialize($allCaps);
              }else{
              $this->request->data['User']['capabilities'] = NULL;
              }
              //$this->request->data['Capability']['user_id'] = $this->request->data['User']['id'];
              //$this->User->Capability->save($this->request->data);
            ...
            /* //Only for capabilities:
              if( !empty($this->request->data['User']['capabilities']) )
              $this->request->data['User']['capabilities'] = unserialize(trim($this->request->data['User']['capabilities'])); */
        }
        $roles = $this->User->Role->find('list');
        $this->set(compact('roles'));
    ...
    public function admin_login() {
        ...
        if ($this->request->is('post')) {
            if ($this->Auth->login()) {
                ...
                //Modified By Reza; //@TODO: Need to find the original way to change the login redirect url;
                //return $this->redirect($this->Auth->redirect());
                $userInfo = $this->Auth->user();
                $redirectUrl = ($userInfo['role_id'] == $this->scmsLevelTeacherRoleId) ? '/admin/' : $this->Auth->redirect();
    ...
    public function index() {
        ...
        if ($this->Employee->hasAny(array('Employee.user_id' => $user_id))) {
            $usertype = 'e';
        }
        /* elseif($this->Employee->hasAny(array('Student.user_id'=>$user_id))){
          $usertype = 's';
          }
          elseif($this->Employee->hasAny(array('Guardian.user_id'=>$user_id))){
          $usertype = 'g';
          } */
    ...
    public function logout() {
        ...
        $this->Session->setFlash(__('লগআউট সফল হয়েছে.'), 'default', array('class' => 'success'));
        //$this->redirect($this->Auth->logout());
