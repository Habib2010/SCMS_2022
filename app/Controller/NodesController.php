...
class NodesController extends AppController {
     ...
     */
    public $uses = array(
        'Node', 'Block', 'Employee', 'StudentCycle', 'Student', 'Attendence',
    );
    ...
    public function admin_recover() {
        if ($this->Node->recover()) {
            $this->Session->setFlash(__('Tree Recovered'), 'default', array('class' => 'success'));
        }
    ...
    public function promoted() {
        ...
        if (!empty($this->request->data['lang'])) {
            $this->Cookie->write('lang', $this->request->data['lang'], false, '30 Days');
        }
//pr();die;
        $this->paginate['Node']['order'] = 'Node.created DESC';
        $this->paginate['Node']['limit'] = Configure::read('Reading.nodes_per_page');
        ...
        $conditions = array('Node.type' => 'notice');
        $fields = array('Node.id', 'Node.title', 'Node.bn_title','Node.path', 'Node.body', 'slug');
        $order = array('Node.id DESC');
        $limit = 5;
        $notices = $this->Node->find('all', array('conditions' => $conditions, 'fields' => $fields, 'order' => $order, 'limit' => $limit));
        $chairmanInfo = $this->Employee->find('first', array('conditions' => array('User.role_id' => 30, 'Employee.status' => 1)));
        $unoInfo = $this->Employee->find('first', array('conditions' => array('User.role_id' => 31, 'Employee.status' => 1)));
        $sochib = $this->Employee->find('first', array('conditions' => array('User.role_id' => 32, 'Employee.status' => 1)));
        $director = $this->Employee->find('first', array('conditions' => array('User.role_id' => 33, 'Employee.status' => 1)));
        $principal = $this->Employee->find('first', array('conditions' => array('User.role_id' => 8, 'Employee.status' => 1)));
        $blockArray = array();
        $blocks = $this->Block->find('all', array('conditions' => array('Block.region_id' => 20), 'recursive' => -1));
        foreach ($blocks as $block) {
            $routine = array();
            $this->Node->recursive = -1;
            $conditions = array('Node.type' => $block['Block']['alias'], 'Node.status' => 1);
            $fields = array('Node.id', 'Node.title', 'Node.bn_title','Node.path', 'Node.excerpt', 'Node.body', 'Node.created', 'slug', 'Node.type');
            $order = array('Node.id DESC');
            $limit = 5;
            $routine = $this->Node->find('all', array('conditions' => $conditions, 'fields' => $fields, 'order' => $order, 'limit' => $limit));
            $routine['slug'] = $this->Cookie->read('lang') == 'bn' ? $block['Block']['bn_title'] : $block['Block']['title'];
            array_push($blockArray, $routine);
        }
        //pr($blockArray);die;
        $this->set(compact('nodes', 'notices', 'chairmanInfo', 'unoInfo', 'sochib', 'director', 'principal', 'routine', 'blockArray'));
    }
    ...
    /* News Headline shows in the Top */
    ...
    function news() {
        ...
        $fields = array('Node.id', 'Node.title', 'Node.bn_title', 'Node.excerpt', 'slug', 'type');
        $order = array('Node.id DESC');
        $limit = 5;
        $news = $this->Node->find('all', array('conditions' => $conditions, 'fields' => $fields, 'order' => $order, 'limit' => $limit));

        return $news;
    }

    /* Notice shows in sidebar */
    ...
    function notice() {
        ...
        $conditions = array('Node.type' => 'notice');
        $fields = array('Node.id', 'Node.title', 'Node.path', 'Node.excerpt', 'Node.created', 'slug');
        $order = array('Node.id DESC');
        $limit = 5;

        $count = $this->Node->find('count', array('conditions' => $conditions, 'order' => $order));
        $notices = $this->Node->find('all', array('conditions' => $conditions, 'fields' => $fields, 'order' => $order, 'limit' => $limit));

        return compact('notices', 'count');
    }
    ...
    function notice_board() {
        ...
        $conditions = array('Node.type' => 'notice');
        $fields = array('Node.id', 'Node.title', 'Node.bn_title', 'Node.path', 'Node.excerpt', 'Node.created', 'slug');
        $order = array('Node.id DESC');
        $limit = 5;
    ...
    /* Exam Routine shows in sidebar */

    function exam_routine() {
        $this->layout = 'empty';
        $this->Node->recursive = 1;

        $conditions = array('Node.type' => 'exam_routine');
        $fields = array('Node.id', 'Node.bn_title', 'Node.title', 'Node.path', 'Node.excerpt', 'Node.created', 'slug');
        $order = array('Node.id DESC');
        $limit = 5;
        $count = $this->Node->find('count', array('conditions' => $conditions, 'order' => $order));
        $notices = $this->Node->find('all', array('conditions' => $conditions, 'fields' => $fields, 'order' => $order, 'limit' => $limit));
    ...
    /* Syllabus shows in sidebar */

    function syllabus() {
        $this->layout = 'empty';
        $this->Node->recursive = 1;

        $conditions = array('Node.type' => 'syllabus');
        $fields = array('Node.id', 'Node.title', 'Node.bn_title', 'Node.path', 'Node.excerpt', 'Node.created', 'slug');
        $order = array('Node.id DESC');
        $limit = 5;
        $count = $this->Node->find('count', array('conditions' => $conditions, 'order' => $order));
    ...
    /* result shows in sidebar */

    function result() {
        $this->layout = 'empty';
        $this->Node->recursive = 1;
        $conditions = array('Node.type' => 'result');
        $fields = array('Node.id', 'Node.title', 'Node.bn_title', 'Node.path', 'Node.excerpt', 'Node.created', 'slug');
        $order = array('Node.id DESC');
        $limit = 5;
        $count = $this->Node->find('count', array('conditions' => $conditions, 'order' => $order));
    ...
    /* class routine shows in sidebar */

    function class_routine() {
        $this->layout = 'empty';
        $this->Node->recursive = 1;
        $conditions = array('Node.type' => 'class_routine');
        $fields = array('Node.id', 'Node.title', 'Node.bn_title', 'Node.path', 'Node.excerpt', 'Node.created', 'slug');
        $order = array('Node.id DESC');
        $limit = 5;
        $count = $this->Node->find('count', array('conditions' => $conditions, 'order' => $order));
