...
class AppController extends Controller {
    ...
    public $scmsClassNames = array('ZERO', 'ELECTRICAL', 'MECHANICAL', 'COMPUTER', 'CIVIL', 'ELECTRONICS', 'TEXTILE', 'ARCHITECTURE','POWER');
    ...
    public function calcGradePoints($total, $subjectTotal, $default) {
        ...
        if ($total_In_100 >= 80)
            return array('A+', 4.00);
        elseif ($total_In_100 >= 75)
            return array('A', 3.75);
        elseif ($total_In_100 >= 70)
            return array('A-', 3.50);
        elseif ($total_In_100 >= 65)
            return array('B+', 3.25);
        elseif ($total_In_100 >= 60)
            return array('B', 3.00);
        elseif ($total_In_100 >= 55)
            return array('B-', 2.75);
        elseif ($total_In_100 >= 50)
            return array('C+', 2.50);
        elseif ($total_In_100 >= 45)
            return array('C', 2.25);
    ...
    protected function limitCapabilities(&$level_id, &$section_id) {
        ...
        if ($userInfo['role_id'] == $this->scmsLevelTeacherRoleId) {
            ...
            $secInfos = $this->Section->find('all', array('conditions' => array('Section.level_id' => $limitCaps['Employee']['level_id']), 'fields' => array('id'), 'recursive' => -1));
            $secId = array();
            foreach ($secInfos as $secInfo):
                $secId[] = $secInfo['Section']['id'];
            endforeach;
            // pr($limitCaps); 
            if (!(empty($limitCaps))) {
                $withCaps = true;
    ...
    protected function sendSMS($type, $recipients, $args = array()) {
        ...
        if ($Scms['cmp'] == 'ON') {
            if (empty($type) || !in_array($type, array('absent', 'halfAbsent', 'general', 'admission', 'result-publish', 'admission-result', 'seat-plan', 'fees', 'generate_fees', 'bkash_fees')) || empty($recipients) || !is_array($recipients))
                return false;
...
//            );
            ...
            if ($type == 'general') {
                ...
                for ($i = 0, $limit = 190; $i < count($recipients); $i+=$limit) {
                ...
                $val = $args['ses'] + 1;
                $ses = $args['ses'] . '-' . $val;
                $adminMsg = $args['level']['Level']['name'] . '-' . $args['grpName'] . '-' . $args['section'] . ':' . $ses . '-(total=' . $countSms . ')';
                //====Guardian's Copy======
                foreach ($recipients as $k => $student) {
                    $params['To'] = preg_replace('/^[0]/', '88$0', $student['Guardian']['mobile']);
                    $params['Message'] =
                            "Dear Parent,
" . date('j-M-y') . " e " . strtoupper($student['Student']['name']) . " Class: " . $args['level']['Level']['name'] . ' e Absent Royeche
#Principal, Aptouch Ploytechnic Institute' . "
$smsFoot";
                ...
                $params['Message'] =
                        "Dear Sir,
Absent SMS requests were sent:" . $adminMsg . " on " . date('j-M-y \a\t h:i:s A') . "
$smsFoot";
...
$smsFoot";
                ...
                foreach ($recipients as $k => $student) {
                    ...
                    $params['Message'] =
                            "Dear Parent,
Please be informed that " . strtoupper($student['Student']['name']) . " Paid Tution Fees on " . date('j-M-y') . "
$smsFoot";
                    // $result1 = $client->call('SendTextMessage', $params);
                }
                //====Admin Notification====:
                //TZS-SIR==>01718078772,Shaheen-Sir==>01725021999,Mahadi==>01722856004,Imran==>01720556561,Reza=>01552326942;
                $params['To'] = $headNumber;
                $params['Message'] =
                        "Dear Sir,
Please be informed that the Fees SMS requests were sent for the class: " . $adminMsg . " on " . date('j-M-y \a\t h:i:s A') . "
$smsFoot";
                //$result2 = $client->call('SendTextMultiMessage', $params);
                $cnt = array_filter(explode(',', $params['To']));
                $countSms+=count($cnt);
                $this->saveSms($countSms, $params['Message'], $type);
                //return $countSms;
            } elseif ($type == 'generate_fees') {
                $countSms = count($recipients);
                $adminMsg = $args['level'] . '-' . $args['section'] . '-(total=' . $countSms . ')';
                //====Guardian's Copy======
                foreach ($recipients as $k => $student) {
                    $mn = date("F", mktime(0, 0, 0, $student['month'], 1, 0));
                    $params['To'] = preg_replace('/^[0]/', '88$0', $student['mobile']);
                    $params['Message'] =
                            "Dear Parent,
Please be informed that Tution Fees of " . strtoupper($student['name']) . " on " . $mn . ' ' . $student['year'] . " is " . $student['total'] . "
$smsFoot";
                    // $result1 = $client->call('SendTextMessage', $params);
                }
                //====Admin Notification====:
                ...
                $params['Message'] =
                        "Dear Sir,
Please be informed that the Generate Fees SMS requests were sent for the class: " . $adminMsg . " on " . date('j-M-y \a\t h:i:s A') . "
$smsFoot";
                //pr($params['Message']); die;
                //$result2 = $client->call('SendTextMultiMessage', $params);
                $cnt = array_filter(explode(',', $params['To']));
                $countSms+=count($cnt);
                //return $countSms;
                $this->saveSms($countSms, $params['Message'], $type);
            } elseif ($type == 'bkash_fees') {
                //====Guardian's Copy======
                foreach ($recipients as $k => $student) {
                    $params['To'] = preg_replace('/^[0]/', '88$0', $student['Guardian'][0]['mobile']);
                    $params['Message'] =
                            "Dear Parent,
Please be informed that " . strtoupper($student['Student']['name']) . " Paid Tution Fees " . $args['total'] . " taka on " . date('j-M-y') . "
$smsFoot";
                    //pr($params['Message']); die;
                    // $result1 = $client->call('SendTextMessage', $params);
                }
                ...
                $params['Message'] =
                        "Dear Sir,
Please be informed that the Bkash Fees SMS requests were sent for the class: " . $args['level'] . " on " . date('j-M-y \a\t h:i:s A') . "
$smsFoot";
                //$result2 = $client->call('SendTextMultiMessage', $params);
                $cnt = array_filter(explode(',', $params['To']));
                $countSms+=count($cnt);
                ...
                $params['To'] = preg_replace('/^[0]/', '88$0', $args['Guardian']['mobile']);
...
trxId: " . $args['trxId'] . "
Class: " . (($args['level'] == 90 || $args['level'] == 91 || $args['level'] == 92) ? $this->scmsClassNames[$args['level']] : $args['level']) . "
$smsFoot" :
                        "Dear Guardian,
...
$smsFoot";
    ...
    protected function chk_bKash_fees($queryStr, $args = array()) {
        if (!empty($queryStr) && !empty($args['reference'])) {
            ...
            $data = array(
                ...
                'pass' => 't3cH9L3Xu5247',
                'msisdn' => '01705806080',
                'reference' => $queryStr //'513132201', ['trxid'||'reference'] 
            );
            /* $data = array(
              'user'				=> 'ESOFTARENA',
              'pass'				=> 'ad0b3tng0',
            ...
            $url = "http://www.bkashcluster.com:9080/dreamwave/merchant/trxcheck/" . $urlPart['reference'];
