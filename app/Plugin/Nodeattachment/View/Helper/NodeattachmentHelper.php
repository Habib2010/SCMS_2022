...
class NodeattachmentHelper extends AppHelper {
       ...
       private function __thumb() {
              ...
              if (file_exists($this->conf['iconDir'] . DS . $thumb_filename)) {
			         ...
			         //pr($file_name);
			         if($file_name[1]=='pdf'){
						$thumb_filenamee = 'thumb_application.png';
					 }
					 elseif($file_name[1]=='doc'){
						$thumb_filenamee = 'thumb_application.png';
					 }
					 elseif($file_name[1]=='flv'){
						$thumb_filenamee = 'thumb_video.png';
					 } 
					 else{
						$thumb_filenamee = 'thumb_default.png';
					 }
			  
                     $thumbpath = 'plugin'.DS.'nodeattachment'.DS.'webroot'.DS.'img';
					 $data['thumb_path'] = ROOT . DS . APP_DIR . DS . $thumbpath;
					 
                     $data['server_thumb_path'] = $data['thumb_path'] . DS .$thumb_filenamee;					 
					 $this->nodeattachment['Nodeattachment'] = $data;
                     //pr($data);
					 return ;
              }
