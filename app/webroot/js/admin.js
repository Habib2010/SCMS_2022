...
function admission_result_sms(row){
    ...
    alert(row);
    $.ajax({                   
        url: '/DPI/admissions/admit_sms_result/' + escape(row),
        cache: false,
        type: 'GET',
        dataType: 'HTML',
...
$(document).ready(function() {
    ...
    $('#UserRoleId').change(function(){
     ...
     $("#dashboard_nav ul, #dashboard_nav li").each(function() { 
        this.className = ""; 
        this.style = "";
		
    });
    $("#dashboard_nav li:has(ul) >a").on('click', function() {
...
function getStudentInfo() {
    ...
    $.ajax({    
        ...
        url: '/DPI/credits/std_session_ajax' ,
        cache: false,
        type: 'GET',
        dataType: 'HTML',
        data : {
...
$(function() {
    $( "#reason").change(function(event) {
        ...
        $.ajax({    
            ...
            url: '/DPI/purposes/reason_purpose_ajax' ,
            cache: false,
            type: 'GET',
            dataType: 'HTML',
            data : {
...
$(function() {
    ...
    $( "#termId").change(function( event ) {
        $("#dvloader").show();
            
        $.ajax({    
                
            url: '/DPI/courses/mark_course_ajax/' +escape( $(this).val()),
            cache: false,
            type: 'GET',
            dataType: 'HTML',
            //data : {"id" :  $(this).val()},
...
$(function() {
    ...
    $( "#session").change(function( event ) {
        ...
        // alert(strUser);
        $.ajax({    
            url: '/DPI/student_results/session_ajax' ,
            cache: false,
            type: 'GET',
            dataType: 'HTML',
            data : {
...
$(function() {
    $( "select.groupId").change(function( ) {
        ...
        var e = document.getElementById("scms-class-result");
        var depId = e.options[e.selectedIndex].value;
        var f = document.getElementById("grSession");
        var sesId = f.options[f.selectedIndex].value;
        $.ajax({    
            url: '/DPI/student_results/group_ajax' ,
            cache: false,
            type: 'GET',
            dataType: 'HTML',
            data : {
...
$(function() {
    $( "select.rgroupId").change(function( ) {
        $("#crs-loading").show();
        var group = $(".rgroupId option:selected").val();
        var e = document.getElementById("scms-class-result");
        var depId = e.options[e.selectedIndex].value;
        var f = document.getElementById("grSession");
        var sesId = f.options[f.selectedIndex].value;
        $.ajax({    
            url: '/DPI/student_results/ref_group_ajax' ,
            cache: false,
            type: 'GET',
            dataType: 'HTML',
            data : {
...
$(function() {
    ...
    $( "#StudentSessionYear").change(function( event ) {
        var e = document.getElementById("StudentSessionYear");
        var strUser = e.options[e.selectedIndex].value;
        //alert(e);
        $.ajax({    
            url: '/DPI/student_results/session_ajax' ,
...
function getStudTestInfos() {
    var std = document.getElementById("testId").value;
    var session = document.getElementById("session").value;
    $("#ajximg").show();
    $.ajax({    
        url: '/DPI/testimonials/std_test_ajax' ,
        cache: false,
        type: 'GET',
        dataType: 'HTML',
        data : {
...
$(document).ready(function() {
    if ($('.mobileMenu').length > 0) {
        $('.mobileMenu').on('click', function() {
            ...
            $('#nav ul.sf-menu').slideToggle('slow');
        });
        if ($(window).width() < 768)
        {
