...
<table cellpadding="0" cellspacing="0">
    ...
    foreach ($settings as $setting) {
        ...
        if ($setting['Setting']['key'] == 'Scms.credit_expire_date' && $setting['Setting']['value'] != 0 && !empty($setting['Setting']['value'])) {
            $val = date('d-m-Y', $setting['Setting']['value']);
        } else {
            $val = $this->Text->truncate($setting['Setting']['value'], 20);
        }
        $rows[] = array(
            $setting['Setting']['id'],
