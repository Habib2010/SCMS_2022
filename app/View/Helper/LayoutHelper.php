...
class LayoutHelper extends AppHelper {
    ...
    public function menu($menuAlias, $options = array()) {
        $_options = array(
            'tag' => 'ul',
            'tagAttributes' => array('class' => 'menu'),
            'selected' => 'selected',
            'dropdown' => false,
...
//                'title' => $lang == 'bn' ? $link['Link']['bn_title'] : $link['Link']['description'],
//                'bn_title' => $link['Link']['bn_title'],
            ...
            $link_title = $options['lang'] == 'bn' ? $link['Link']['bn_title'] : $link['Link']['title'];
            $linkTitle = $this->Html->tag('span', $link_title);
            $linkOutput = $this->Html->link($linkTitle, $link['Link']['link'], $linkAttr);
            if (isset($link['children']) && count($link['children']) > 0) {
                $linkOutput .= $this->nestedLinks($link['children'], $options, $depth + 1);
                $current = strpos($linkOutput, ' selected');
            }
            if ($current && $depth == 1) {
                $linkOutput = $this->Html->tag('li', $linkOutput, array('class' => 'current'));
            } else {
        ...
        if ($output != null) {
            ...
            if ($depth > 1) {
                $output = $this->Html->tag('div', $output);
            }
        }
    ...
    public function metaField($key = '', $value = null, $id = null, $options = array()) {
        ...
        $actions = $this->Html->link(
                __('Remove'), is_null($id) ? '#' : array('plugin' => null, 'controller' => 'nodes', 'action' => 'delete_meta', $id), array('class' => 'remove-meta', 'rel' => $id)
        );
    ...
    public function adminTabsType($show = null, $types) {
        if (!isset($this->adminTabs)) {
            $this->adminTabs = false;
        }
        $output = '';
        $typee = $types['Type']['is_event'];
        ...
        if (is_array($tabs)) {
            foreach ($tabs as $title => $tab) {
                if (!isset($tab['options']['type']) || (isset($tab['options']['type']) && (in_array($this->_View->viewVars['typeAlias'], $tab['options']['type'])))) {
                    $domId = strtolower(Inflector::singularize($this->params['controller'])) . '-' . strtolower(Inflector::slug($title, '-'));
                    if ($this->adminTabs) {
                        if ($typee == 1 and $domId == 'node-event-date') {
                            if (strstr($tab['element'], '.')) {
                                $elementE = explode('.', $tab['element']);
                                $plugin = $elementE['0'];
                                $element = $elementE['1'];
                            ...
                            $output .= '</div>';
                        } elseif ($typee == 0 and $domId == 'node-event-date') {
                        ...
                        } elseif ($typee == 1 and $domId != 'node-event-date') {
                            if (strstr($tab['element'], '.')) {
                                $elementE = explode('.', $tab['element']);
                                $plugin = $elementE['0'];
                                $element = $elementE['1'];
                        ...
                        //$output .= '<li><a href="#' . $domId . '">' . $title . '</a></li>';
                        //echo $domId = strtolower(Inflector::singularize($this->params['controller'])) . '-' . strtolower(Inflector::slug($title, '-'));
                        if ($typee == 1 and $domId == 'node-event-date') {
                            $output .= '<li><a href="#' . $domId . '">' . $title . '</a></li>';
                        } elseif ($typee == 0 and $domId == 'node-event-date') {
