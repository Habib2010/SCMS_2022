...
$grpid=$this->request->data['Text']['group_id'];
 $grp=array(
     $grpid=>$this->request->data['Group']['name']
         );
 $session=$this->request->data['Text']['school_session_id'];
 $term=$this->request->data['Text']['school_term_id'];
...
?>
    <div class="blockWrapper">
        <div class="block">
            ...
            <div class="set_wrapper">
            <?php 
            if(empty($this->request->data['Text']['group_id'])){
            echo $this->Form->input('group_id', array('id' => 'scms-group', 'label' => 'Semister :', 'empty' => '--Select--', 'class' => 'updateDrops', 'data-belongs' => '#scms-course-3rd,#scms-course-4th', 'data-depends' => 'scms-class')); 
            }else {
                echo $this->Form->input('group_id', array('id' => 'scms-group', 'label' => 'Semister :', 'empty' =>$grp, 'class' => 'updateDrops', 'data-belongs' => '#scms-course-3rd,#scms-course-4th', 'data-depends' => 'scms-class')); 
            }
            ?>
            </div><!-- end of set_wrapper-->
            ...
            <div class="set_wrapper">
                <?php
                if(!empty($term)){
                    echo $this->Form->input('term_id', array('id' => 'scms-term', 'label' => '** Term', 'selected' => $term));
                }else {
                  echo $this->Form->input('term_id', array('id' => 'scms-term', 'label' => '** Term', 'empty' => 'Select Term'));
                }
                ?>
            </div><!-- end of set_wrapper-->
            <div class="set_wrapper">
                ...
                <?php foreach ($schoolSessions as $schoolSession):
                    ...
                    if ($session == $schoolSession['SchoolSession']['id'])
                        {
                    ...
                    ?>
                    <option value="<?php echo $schoolSession['SchoolSession']['id'];?>" <?php echo $selected;?>><?php echo $schoolSession['SchoolSession']['id'].'-'.$val; ?></option>
                <?php endforeach;?>
                </select>
            </div><!-- end of set_wrapper-->
            <div class="set_wrapper">
