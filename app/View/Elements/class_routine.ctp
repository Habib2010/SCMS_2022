<?php 
	$notice = $this->requestAction('nodes/class_routine');
	$count = $this->requestAction('nodes/class_routine'); 
?>
        <ul>
		<?php 
		...
		/**For Year Display**/
			...
			?>
		<li><a href ="<?php echo $this->request->base?><?php echo $not['Node']['path'];?>"><span class="acc_title"><span><?php echo $date_res; echo $month_res;?></span><?php echo $year_res;?></span><?php echo $not['Node']['title'];?></a></li>
		<?php
		endforeach ;
		}
		endforeach ; ?>
	...
	<?php if($count['count']>5){ ?>
		<div class="lwrLink">
			<a href="<?php echo $this->request->base ?>/class_routine" class="lwrLink">সবগুলো জানতে &raquo;</a>	
		</div>
	<?php } ?>
