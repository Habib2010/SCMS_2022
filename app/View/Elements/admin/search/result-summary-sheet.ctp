<div class="formTopWrapper">
    <div class="displayWrapper"<?php if (empty($students) || !empty($this->request->data)) echo ' style="display:block"'; ?>>
        <?php echo $this->Form->create(null, array('url' => array('controller' => 'result_summaries', 'action' => 'index'))); ?>
        ...
        <div class="set_wrapper">
            <div class="infofoset itemfirstset">
                ...
                <select name="data[StudentResult][school_session_id]">
                    <option value="">Select Session</option>
                    <?php
                    foreach ($schoolSessions as $schoolSession):
                        $val = $schoolSession['SchoolSession']['id'] + 1;
                        ?>
                        <option value="<?php echo $schoolSession['SchoolSession']['id']; ?>" 
                        <?php
                        if (!empty($sesId)) {
                            if ($schoolSession['SchoolSession']['id'] == $sesId)
                                echo ' selected="selected"';
                        }else {
                            if ($schoolSession['SchoolSession']['id'] == date('Y'))
                                echo ' selected="selected"';
                        }
                        ?>>
                                    <?php echo $schoolSession['SchoolSession']['id'] . '-' . $val; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div><!-- end of infofoset-->
            <div class="infofoset">
                ...
                echo $this->Form->input('sort', array('label' => 'Sort By', 'selected' => 'roll', 'options' => $sort));
                ?>
            </div><!-- end of infofoset-->
            <div class="infofoset itemlastset">
